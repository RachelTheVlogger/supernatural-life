...
import { X, Video, Users, Eye, TrendingUp, DollarSign, Play, MessageCircle, ThumbsUp, Bell, Award, Flame, Zap, AlertTriangle, Shield } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-red-300 font-medium">
                  {stalkers.filter(s => s.platform === 'youtube' && !s.blocked && s.danger_level !== 'harmless').length} YouTube Stalker{stalkers.filter(s => s.platform === 'youtube' && !s.blocked && s.danger_level !== 'harmless').length > 1 ? 's' : ''}
                </span>
              </div>
          ...
          <button onClick={() => setTab('stalkers')} className={`px-4 py-2 rounded-lg whitespace-nowrap text-sm relative ${tab === 'stalkers' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
            ...
            {stalkers.filter(s => s.platform === 'youtube' && !s.blocked && s.danger_level !== 'harmless').length > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[10px] rounded-full flex items-center justify-center">
                {stalkers.filter(s => s.platform === 'youtube' && !s.blocked && s.danger_level !== 'harmless').length}
              </span>
            )}
          </button>
        ...
        {tab === 'stalkers' && (
          <div className="space-y-4 max-h-[55vh] overflow-y-auto">
            {stalkers.filter(s => s.platform === 'youtube').length === 0 ? (
              <div className="text-center py-12">
                <Shield className="w-16 h-16 text-green-400 mx-auto mb-4 opacity-50" />
                <p className="text-gray-400">No stalkers on YouTube yet.</p>
              </div>
            ) : (
              stalkers.filter(s => s.platform === 'youtube').map(stalker => {
                const DANGER_COLORS = {
                  ...
                  <div key={stalker.id} className="bg-gray-800 rounded-xl p-4 border-2 border-red-900/30">
                    ...
                    {stalker.behavior_patterns?.length > 0 && (
                      <div className="mb-3 bg-black/40 rounded p-2">
                        <p className="text-red-300 text-xs font-medium mb-1">Behavior:</p>
