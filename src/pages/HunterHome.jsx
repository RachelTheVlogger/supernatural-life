...
export default function HunterHome() {
  ...
  const [showJointMissions, setShowJointMissions] = useState(false);
  const [showJealousy, setShowJealousy] = useState(false);
    ...
    <div className="min-h-screen relative p-4 md:p-6 pb-24 overflow-x-hidden" style={{
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          {showDomestic && selectedVampire && (
            <HunterDomesticRoutines
              hunter={hunter}
              vampire={selectedVampire}
              onClose={() => setShowDomestic(false)}
            />
          )}

          {showTrust && selectedVampire && (
            <HunterSecretsAndTrust
              hunter={hunter}
              vampire={selectedVampire}
          ...
          {showMoral && selectedVampire && (
            <HunterMoralDilemmas
              hunter={hunter}
              vampire={selectedVampire}
              onClose={() => setShowMoral(false)}
            />
          )}

          {showJointMissions && selectedVampire && (
            <HunterJointMissions
              hunter={hunter}
              vampire={selectedVampire}
              onClose={() => setShowJointMissions(false)}
            />
          )}

          {showJealousy && selectedVampire && (
            <HunterJealousyEvents
              hunter={hunter}
              vampire={selectedVampire}
              onClose={() => setShowJealousy(false)}
            />
          )}

          {showHousehold && selectedVampire && (
            <HunterHouseholdDecisions
              hunter={hunter}
              vampire={selectedVampire}
          ...
          {showEscape && selectedVampire && (
            <HunterEscapePlan
              hunter={hunter}
              vampire={selectedVampire}
              onClose={() => setShowEscape(false)}
            />
          )}

          {showFeeding && selectedVampire && (
            <HunterFeedingControl
              hunter={hunter}
              vampire={selectedVampire}
          ...
          {showIntegration && selectedVampire && (
            <HunterIntegration
              hunter={hunter}
              vampire={selectedVampire}
